...
class FrontendArticlesCatalogues_Model extends FC_Model{
	...
	public function ReadAllByField($field = '', $value = 0, $lang = 'vietnamese'){
		...
		$this->db->from('articles_catalogues');
		if (is_array($field)) {
			$this->db->where(array_merge($field, array('alanguage' => $lang)));
		}else{
	...
	public function ReadArticles($cataloguesid = 0, $limit = 4){
		$this->db->select('id, title, slug, canonical, images, description, created, viewed');
		$this->db->where(array('trash' => 0,'publish' => 1,'cataloguesid' => $cataloguesid));
		$this->db->from('articles');
		$this->db->limit($limit, 0);
	...
	public function _count($param = '', $catalogue = '', $lang = 'vietnamese'){
		...
		$param['where'] = (isset($param['where'])) ? ' AND '.$param['where'] : '';
		$count = $this->db->query('
			SELECT '.$param['select'].'
			FROM `'.$param['modules'].'` as `pr`
			WHERE `pr`.`trash` = 0 AND `pr`.`alanguage` = \''.$lang.'\' AND `pr`.`publish` = 1'.$param['where'].'
		')->num_rows();
		$this->db->flush_cache();
		return $count;
	}
	...
	public function _view($param = '', $catalogue = '', $lang = 'vietnamese'){
		...
		if (!empty($data)) {
			foreach ($data as $key => $value) {
				$data[ $key ]['sub'] = $this->ReadArticles($value['id'],$param['limitsub']);
			}
		}
		$this->db->flush_cache();
		return $data;
