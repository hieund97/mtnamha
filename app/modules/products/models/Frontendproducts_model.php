...
// Lấy thuộc tính của sản phẩm
	...
	public function countajaxproduct($attribute = '', $cataloguesid = 0, $lang = 'vietnamese'){
  		...
  		if($cataloguesid > 0){
	   		...
	   		if(isset($_list_child) && is_array($_list_child) && count($_list_child)){
	    		foreach($_list_child as $key => $val){
	     			$relationship['query'] = $relationship['query'].(empty($relationship['query'])?('`pr`.`cataloguesid` = '.$val['id']):(' OR `pr`.`cataloguesid` = '.$val['id']));
	    		}
	   		}
	   		else
	   		{
	    		$relationship['query'] = $relationship['query'].(empty($relationship['query'])?('`pr`.`cataloguesid` = '.$catalogue['id']):(' OR `pr`.`cataloguesid` = '.$catalogue['id']));
	   		}
	  	}
	  	$relationship['query'] = (!empty($relationship['query'])?' AND ('.$relationship['query'].')':'');
	  // ON `pr`.`cataloguesid` = `rls`.`cataloguesid`
	  	$count = $this->db->query('
			...
			FROM `attributes_relationship` as `att`
	   		...
	   		WHERE `pr`.`trash` = 0 AND `pr`.`alanguage` = \''.$lang.'\' AND `att`.`productsid` = `pr`.`id`'.$attribute['query'].$relationship['query'].' GROUP BY `att`.`productsid`'.$attribute['having'].' ORDER BY `pr`.`id`'.(isset($order_by) ? $order_by : 'desc').' ')->num_rows();
	  		$this->db->flush_cache();
 	...
 	public function viewajaxproduct($start = 0, $limit = 0, $attribute = '', $cataloguesid = 0, $lang = 'vietnamese'){
	  	...
	  	$relationship['query'] = (!empty($relationship['query'])?' AND ('.$relationship['query'].')':'');
	  	 // ON `pr`.`cataloguesid` = `rls`.`cataloguesid`
	  	$count = $this->db->query('
			SELECT `pr`.`id`, `pr`.`title`, `pr`.`slug`,`pr`.`catalogues`, `pr`.`canonical`, `pr`.`images`, `pr`.`isfooter`, `pr`.`price`, `pr`.`saleoff`, `pr`.`banner`, `pr`.`description`,  `att`.`productsid`, COUNT(`productsid`) as `total`
			FROM `attributes_relationship` as `att`
			INNER JOIN `products` as `pr` ON `pr`.`id` = `att`.`productsid`
			WHERE `pr`.`trash` = 0 AND `pr`.`alanguage` = \''.$lang.'\' AND `att`.`productsid` = `pr`.`id`'.$attribute['query'].$relationship['query'].' GROUP BY `att`.`productsid`'.$attribute['having'].' ORDER BY `pr`.`id`'.(isset($order_by) ? $order_by : 'desc').' LIMIT '.($start * $limit).', '.$limit.' ')->result_array();
			$this->db->flush_cache();
			return $count;
 	}
	...
	/*public function countajaxproduct($attribute = '', $cataloguesid = 0, $lang = 'vietnamese'){
	  ...
	  // ON `pr`.`cataloguesid` = `rls`.`cataloguesid`
	  	$count = $this->db->query('
			...
			FROM `attributes_relationship` as `att`
	   		INNER JOIN `products` as `pr` ON `pr`.`id` = `att`.`productsid`
			INNER JOIN `catalogues_relationship` as `rls` 
 	...
 	public function viewajaxproduct($start = 0, $limit = 0, $attribute = '', $cataloguesid = 0, $lang = 'vietnamese'){
	  	...
	  	$relationship['query'] = (!empty($relationship['query'])?' AND ('.$relationship['query'].')':'');
	  	 // ON `pr`.`cataloguesid` = `rls`.`cataloguesid`
	  	$count = $this->db->query('
			SELECT `pr`.`id`, `pr`.`title`, `pr`.`slug`,`pr`.`catalogues`, `pr`.`canonical`, `pr`.`images`, `pr`.`price`, `pr`.`saleoff`, `pr`.`banner`, `pr`.`description`,  `att`.`productsid`, COUNT(`productsid`) as `total`
			FROM `attributes_relationship` as `att`
			INNER JOIN `products` as `pr` ON `pr`.`id` = `att`.`productsid`
			INNER JOIN `catalogues_relationship` as `rls`
	...
	public function ReadAll($_list_id = '', $lang = 'vietnamese'){

		$this->db->where(array('products.trash' => 0, 'products.alanguage' => $lang));
		$this->db->select('products.id,products.catalogues,products.title,images,description,price,saleoff,canonical,slug');
