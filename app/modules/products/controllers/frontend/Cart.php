...
//			$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email');
			...
			if($this->form_validation->run($this)){
				$_paymentid = $this->Autoload_Model->_create(array(
					...
					'data' => array(
						...
						'send' => 0,
						'created' => gmdate('Y-m-d H:i:s', time() + 7*3600),
                        'userid_created' => ((isset($customer) && is_array($customer)) ? $customer['id'] : ''),
					),
				));
				if($_paymentid > 0){
					if(isset($cart) && is_array($cart) && count($cart)){
	...
	public function success(){
		...
		if ($this->session->has_userdata('cauhinh')) {
			$this->session->unset_userdata('cauhinh');
		}
		if ($this->session->has_userdata('pc_part_id')) {
			$this->session->unset_userdata('pc_part_id');
		}
		if ($this->session->has_userdata('pc_total_price')) {
			$this->session->unset_userdata('pc_total_price');
		}
		...
		if($this->input->post('confirm')){
			...
			if(isset($_product_) && !empty($_product_)){
				...
				$this->mailbie->sent(array(
					'to' => $_payment['email'],
					'cc' => $this->fcSystem['contact_email'],
